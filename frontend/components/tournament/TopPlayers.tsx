...
const TopPlayers = ({ featuredEvent }: TopPlayerProps) => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (!data.topPlayers.rating)
                        settopPlayersFilter('goals')
                    else
                        settopPlayersFilter('rating')
    ...
    if (waitdata == 'error')
        return <div className="">{featuredEvent?.tournament.uniqueTournament.id}:{featuredEvent?.season.id}</div>
    if (waitdata == 'wait' || !topPlayers)
        return <div className="MYDeg  w-full bg-[#ffffff] rounded-2xl  space-y-2 ">
    ...
    return (
        <div className="MYDeg  w-full bg-[#ffffff] rounded-2xl  space-y-2 ">
                                    ...
                                    <div  >
                                        <Link href={`/ma/player/${data.player.slug}/${data.player.id}`} className="flex space-x-2 items-center  px-2 text-gray-900 text-opacity-75 hover:bg-custom-default-hover">
                                            ...
                                            <div className=" w-14  relative">
                                                ...
                                                <Image className=" absolute  bottom-0 right-0 w-5" width={500} height={500} src={`https://sofascore.com/api/v1/team/${data.team?.id}/image`} alt="" />
                                            </div>
                                            <div className={`w-full flex   justify-between items-center p-2  `}>
                                                <div className=" flex flex-col justify-center ">
                                                    <p className='text-black  font-normal'>{data.player.name}</p>
                                                    <p className="text-gray-900 opacity-35 text-[12px] truncate">{data.team?.shortName} </p>
                                                </div>
            ...
            <div className={` w-full p-3 flex space-x-2  justify-end items-center`}>
