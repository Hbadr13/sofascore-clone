            ...
            {!isSelected ? <div className="space-y-6">
                {
                    ...
                    ['F', 'M', 'D', 'G'].map((position, index) =>
                        <div key={index} className="">
                            ...
                            <div className=" grid grid-cols-1 tablet:gap-4    tablet:grid-cols-4 desktop:grid-cols-6">
                                {
                                    allPlayers.players.map((player, ind) =>
                                        player.player.position == position &&
                                        <div key={ind} className=' w-full tablet:w-[134px] bg-surface-s2 rounded-xl flex flex-col justify-between'>
                                            <div className="flex  flex-row  tablet:flex-col items-center p-4  space-x-2 space-y-0 tablet:space-x-0 tablet:space-y-2">
                                                <div className="relative">
                                                    <DisplayImage onErrorImage='player' className=' w-10 h-10  tablet:w-16  tablet:h-16 rounded-full border-1.5 border-slate-300' width={500} height={500} alt={'player:' + player.player.shortName} src={`https://api.sofascore.app/api/v1/player/${player.player.id}/image`} />
                                                    {player.player.jerseyNumber && <div className=' hidden tablet:flex bottom-0  right-0 absolute w-6 h-6 bg-gray-600 rounded-full  justify-center items-center text-white font-semibold'>{player.player.jerseyNumber}</div>}
                                                </div>
                    ...
                    </div>
                    {
                        ['F', 'M', 'D', 'G'].map((position, index) =>
                            <div key={index} className=' border-t-1'>
                                {
                                    allPlayers.players.map((player, ind) =>
                                        player.player.position == position && <div key={ind} className=' flex  border-b-1 '>
                                            <div className="w-3/5 flex   items-center p-1">
